...
class _EventDetailPageState extends State<EventDetailPage> {
  ...
  bool _isAuthenticated(CookieRequest request) {
    ...
    // Google Sign-In flow sets Django session cookies via request.post(),
    // so CookieRequest.loggedIn may remain false.
  ...
  Future<void> _fetchUserId(CookieRequest request) async {
    ...
    final url =
        "https://tristan-rasheed-court-finder.pbp.cs.ui.ac.id/auth/get-user";
  ...
  Future<void> _handleEventAction(CookieRequest request, bool isJoining) async {
    ...
    final url =
        "https://tristan-rasheed-court-finder.pbp.cs.ui.ac.id/event_list/$endpoint/${widget.event.pk}/";
      ...
      if (response['status'] == 'success') {
        ...
        if (message.contains("already joined")) {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text("Sinkronisasi: Anda sudah terdaftar."),
              ),
            );
          }
  ...
  Widget build(BuildContext context) {
    ...
    String dateStr =
        "${fields.scheduledDate.day}-${fields.scheduledDate.month}-${fields.scheduledDate.year}";
                      ...
                      onPressed: () {
                        ...
                        if (_isLoadingUser || _userId == null) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                              content: Text(
                                "Sedang memuat data pengguna, coba lagi sebentar.",
                              ),
                            ),
                          );
                          return;
                        ...
                        if (currentParticipants.length >= 10 && !isJoined) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(content: Text("Event Penuh!")),
                          );
                          return;
                        }
